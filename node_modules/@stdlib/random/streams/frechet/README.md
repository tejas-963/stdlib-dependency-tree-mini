...
#### randomStream( alpha, s, m\[, options] )

Returns a [readable stream][readable-stream] for generating pseudorandom numbers drawn from a [Fréchet][frechet] distribution with parameters `alpha` (shape), `s` (scale), and `m` (location).
...
stream = randomStream( 2.0, 5.0, 3.0 );
...
var stream = randomStream( 2.0, 5.0, -1.0, opts );
```
...
var stream = randomStream( 2.0, 5.0, 0.0, opts );
...
var stream = randomStream( 2.0, 5.0, 3.0, opts1 );
...
stream = randomStream( 2.0, 5.0, 3.0, opts3 );
...
```javascript
var stream = randomStream( 2.0, 5.0, -1.0 );
...
```javascript
var stream = randomStream( 2.0, 5.0, 3.0 );
...
#### randomStream.factory( \[alpha, s, m, ]\[options] )

Returns a `function` for creating [readable streams][readable-stream] which generate pseudorandom numbers drawn from a [Fréchet][frechet] distribution.
...
```javascript
var createStream = randomStream.factory( 2.0, 5.0, 3.0 );
...
var stream1 = createStream( 2.0, 5.0, 3.0 );
var stream2 = createStream( 2.0, 5.0, 3.0 );
// ...
```
...
#### randomStream.objectMode( alpha, s, m\[, options] )
